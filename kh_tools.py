...
def kh_getSliceImages_simple(
    sBaseImageFiles="", ndSliceSize=(10, 10), nStride=1, bSaveImages=False
):
    lst_pics = []
    lst_names = []

    for sImagePath in sBaseImageFiles:
        lst_picTmp, lst_nameTmp = kh_extractPatchesOne(
            sImagePath,
            nStride=nStride,
...
def kh_getSliceImages(
    ...
    while (i + 6) < len(sBaseImageFiles):
        ...
        lst_picTmp, lst_nameTmp = kh_extractPatches(
            sImagePath,
            nStride=nStride,
            ndSliceSize=ndSliceSize,
            bSaveImages=bSaveImages,
...
def kh_getImages(
    ...
    if bGetSlice:
        return kh_getSliceImages(
            sBaseImageFiles=sBaseImageFiles,
            ndSliceSize=ndSliceSize,
            nStride=nStride,
            bSaveImages=bSaveImages,
        )
